...
if (!file_exists(source_meta_filename)) {
	...
	show_debug_message("Bufferstore: Created new bufferstore FAT: " + source_meta_filename + " blocksize: "+string(init_blocksize));
} else {
	show_debug_message("Bufferstore: Loaded bufferstore FAT: " + source_meta_filename + " blocksize: "+string(init_blocksize));	
}
...
if (check != "bsm") {
	show_debug_message("Bufferstore: Not a bufferstore FAT: " + source_meta_filename + "("+check+")");
	instance_destroy();
	return false;
}

blocksize = real(file_text_read_string(_meta_file)); file_text_readln(_meta_file);
show_debug_message("Bufferstore: Loaded blocksize: " + string(blocksize));
...
show_debug_message("Bufferstore: DB Contains blocks: " + string(blocks));
...
show_debug_message("Bufferstore: FATMAP: " + string(ds_map_size(_fat_map)));
...
show_debug_message("Bufferstore: EMPTYLIST: " + string(ds_list_size(_empty_list)));
/*
if (blocks == 0) {
